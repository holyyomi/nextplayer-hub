...
export default function Calendar() {
  ...
  const selectedEvents = eventsByDate[format(selected, "yyyy-MM-dd")] ?? [];
          ...
          {days.map((day) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "min-h-[112px] border-b border-r p-2 text-left transition-colors hover:bg-muted/30",
                  !inMonth && "bg-muted/10 text-muted-foreground/50",
                  isSelected && "bg-accent/40 ring-2 ring-inset ring-primary/40",
                )}
              >
                <div
                  className={cn(
                    "num mb-1 inline-flex h-6 min-w-6 items-center justify-center rounded-full px-1.5 text-xs",
      ...
      <div className="space-y-4">
        <div className="surface-card p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">{format(selected, "M월 d일 (EEE)", { locale: ko })} 일정</h3>
          </div>
          {selectedEvents.length === 0 ? (
            <div className="rounded-xl border border-dashed py-8 text-center text-xs text-muted-foreground">
              등록된 일정이 없습니다.
              {isAdmin && (
                <Button variant="link" size="sm" className="mt-1 block w-full" onClick={() => openNew()}>
                  + 일정 추가
                </Button>
              )}
            </div>
        ...
        <div className="surface-card p-5">
          <h3 className="mb-3 text-sm font-semibold">다가오는 일정</h3>
          <div className="space-y-2">
            {upcoming.map((e) => (
              <button
                key={e.id}
